<main class="frames-main-60">
    <section class="frames-section-60">
        <div class="frames-image-60">
            <img class="frames-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/svg-01.svg" alt="Title">
        </div>
    </section>
    <section class="dual-section-60">
        <div class="dual-grid-60">
            <div class="dual-left-column-62">
                <div class="dual-content-60">
                    <div class="dual-description-60">
                        We work with leading suppliers across the world to
                        offer museum-grade craftmanship for every project.
                        <br><br>
                        Our curated librery of mouldings spans sleek Italian lacquers
                        to deep-grained woods.<br>
                        Each sample is hand selected for its finish, tone
                        and integrity.
                        <br><br>
                        Explore our color-coded corner wall or schedule a
                        private consultation to find the perfect match for
                        your project.<br>
                        Each season, our design team curates a selection of
                        mouldings and finishes that reflect the current
                        trends in art and interiors.
                    </div>
                </div>
            </div>
            <div class="dual-right-column-62">
                <div class="dual-right-image-60">
                    <picture>
                        <source srcset="<?= BASE_URL; ?>public/assets/image/frames/img-02.png" media="(max-width: 678px)">
                        <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/img-02.png" alt="Frame Image">
                    </picture>
                </div>
            </div>
        </div>
    </section>
    <section class="frames-title-section-60">
            <h1 class="frames-title-60">DISCOVER OUR LASTEST FRAMES</h1>
    </section>
    <section class="dual-section-60">
        <div class="dual-grid-60">
            <div class="dual-left-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/745037.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 745037</h3>
                    <p class="dual-title-60">Roma Lavo</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/745037">Specs</a>
                </div>
            </div>
            <div class="dual-right-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/535046.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 535046</h3>
                    <p class="dual-title-60">Roma Lavo</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/535046">Specs</a>
                </div>
            </div>
        </div>
    </section>
    <section class="dual-section-60">
        <div class="dual-grid-60">
            <div class="dual-left-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/869048.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 869048</h3>
                    <p class="dual-title-60">Roma Lavo</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/869048">Specs</a>
                </div>
            </div>
            <div class="dual-right-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/606052.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 606052</h3>
                    <p class="dual-title-60">Roma Eleganza</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/606052">Specs</a>
                </div>
            </div>
        </div>
    </section>
    <!-- <section class="dual-section-60">
        <div class="dual-grid-60">
            <div class="dual-left-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/752.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 745037</h3>
                    <p class="dual-title-60">Roma Lavo</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/745037">Specs</a>
                </div>
            </div>
            <div class="dual-right-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/203299.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 745037</h3>
                    <p class="dual-title-60">Roma Lavo</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/745037">Specs</a>
                </div>
            </div>
        </div>
    </section> -->
    <!-- <section class="dual-section-60">
        <div class="dual-grid-60">
            <div class="dual-left-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/253552.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 745037</h3>
                    <p class="dual-title-60">Roma Lavo</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/745037">Specs</a>
                </div>
            </div>
            <div class="dual-right-column-60">
                <div class="dual-image-60">
                    <img class="dual-img-60" src="<?= BASE_URL; ?>public/assets/image/frames/303800.png" alt="Frame Image">
                </div>
                <div class="dual-content-60">
                    <h3 class="dual-header-60">Frame 745037</h3>
                    <p class="dual-title-60">Roma Lavo</p><br>
                    <a class="dual-description-60" href="<?= BASE_URL; ?>frame/745037">Specs</a>
                </div>
            </div>
        </div>
    </section> -->
</main>